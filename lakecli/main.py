...
from lakecli.packages.prompt_utils import confirm, confirm_destructive_query
from lakecli.key_bindings import cli_bindings
from lakecli.clitoolbar import create_toolbar_tokens_func
...
LAKE_CLI_RC = '~/.lakecli/lakeclirc'
DEFAULT_CONFIG_FILE = os.path.join(PACKAGE_ROOT, 'lakeclirc')
...
For more details about the error, you can check the log file: %s''' % (LAKE_CLI_RC, _cfg['main']['log_file'])
            self.echo(err_msg)
    ...
    def init_logging(self, log_file, log_level_str):
        ...
        root_logger = logging.getLogger('lakecli')
        root_logger.addHandler(handler)
        root_logger.setLevel(log_level)

        root_logger.debug('Initializing lakecli logging.')
...
def cli(execute, region, aws_access_key_id, aws_secret_access_key,
    ...
    Examples:
      ...
      - lakecli my_database
    '''
    if (lake_cli_rc == LAKE_CLI_RC) and (not os.path.exists(os.path.expanduser(LAKE_CLI_RC))):
        err_msg = '''
        Welcome to lakecli!

        It seems this is your first time to run lakecli,
        we generated a default config file for you
            %s
        Please change it accordingly, and run lakecli again.
        ''' % LAKE_CLI_RC
        print(err_msg)
        write_default_config(DEFAULT_CONFIG_FILE, LAKE_CLI_RC)
        sys.exit(1)
